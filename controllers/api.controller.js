...
module.exports.justifyText = function(req, res) {
    ...
    checkAuthorizationToken(req.headers).then(
        (user) => {
            ...
            let wordsNbr = (text.split(/ |\r|\n/)).length;

            if (wordsNbr > wordNbrLimit ||
                (user.quotas.hasOwnProperty(today) && user.quotas[today] + wordsNbr > wordNbrLimit)) {
            ...
            for (let i = 0; i < nbWords; i++) {
                ...
                if (word.indexOf('\n') > -1) {
                    ...
                    for (let j = 0; j < innerWordsLength; j++) {
                        ...
                        if (innerWord === '') {
                            ...
                            else if (currentLine.length + 1 + innerWord.length <= lineWidth) {
                                currentLine += ' ' + innerWord;
                                lines.push(currentLine);
                ...
                if (word.indexOf('\n') == -1) {
                    ...
                    // handle words with length greater tha lineWidth
                    if (word.length > lineWidth) {
                        ...
                        let remainingCharactersNumber = lineWidth - currentLine.length - (currentLine.length == 0 ? 1 : 2);
                        ...
                        while (remainingCharactersNumber > 0 && word.length != 0) {
                            ...
                            remainingCharactersNumber = lineWidth - currentLine.length - (currentLine.length == 0 ? 1 : 2);
                        }
                    ...
                    // handle 'normal' words
                        ...
                        if (currentLine.length + word.length + (currentLine.length == 0 ? 0 : 1) <= lineWidth) {
            ...
            // if user have made requests today, add the wordsNbr to his quota
            if (quotas.hasOwnProperty(today))
                quotas[today] += wordsNbr;
            // else create the user initialize today quota with the wordsNbr
            else
                quotas[today] = wordsNbr;
...
function justifyLine(line) {
    ...
    let nbRemainedCharacters = lineWidth - lineLength;
